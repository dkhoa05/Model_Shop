...
export function getServerOrigin() {
  ...
  if (/^https?:\/\//i.test(base)) {
    ...
    // Dev: ảnh /uploads đi qua proxy Vite (cùng origin :5173/:5174), tránh gọi trực tiếp :5000 (hay bị chặn / lỗi)
    if (
      import.meta.env.DEV &&
...
/**
 * Ảnh tĩnh phục vụ tại `/uploads` (không phải `/api/uploads`).
 * Chuẩn hóa URL cũ/sai lưu trong DB.
 */
...
/** Dev: URL đầy đủ http://localhost:5000/uploads/... → chỉ /uploads/... để proxy Vite phục vụ */
