...
export default function ProductsPage() {
  ...
  const addToCart = (p) => {
    ...
    setToast("Đã thêm mô hình vào giỏ hàng.");
    window.clearTimeout(addToCart._t);
    addToCart._t = window.setTimeout(() => setToast(""), 1500);
  };
                ...
                {filtered.map((p) => (
                  <Link key={p._id} to={`/products/${p._id}`} className={cardClass}>
                    <div className="aspect-square bg-slate-100 dark:bg-slate-800/80 overflow-hidden relative">
                      ...
                      <span
                        className={
                          ...
                          getAvailabilityBadgeClass(p.availability || "in_stock", theme)
                        }
                      >
                        {AVAILABILITY_LABELS[p.availability] || AVAILABILITY_LABELS.in_stock}
                      </span>
                      <img
                    ...
                    <div className="p-3 sm:p-4 space-y-1">
                      ...
                      >
                        {canAddModelToCart(p) ? "Thêm vào giỏ" : "Không thể đặt"}
                      </button>
                    </div>
                  </Link>
                ))}
