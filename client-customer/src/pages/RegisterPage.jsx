...
export default function RegisterPage() {
  ...
  const afterRegister = useMemo(() => safeRedirectPath(searchParams.get("redirect")), [searchParams]);
  const { user } = useAuth();
  const { theme } = useTheme();

  useEffect(() => {
    if (user) navigate(afterRegister, { replace: true });
  }, [user, navigate, afterRegister]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setTimeout(
        () => navigate(`/login?redirect=${encodeURIComponent(afterRegister)}`),
        1000
      );
    } catch (err) {
      ...
      <div className="w-full grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12 items-center">
        ...
        <div className="max-w-sm mx-auto md:mx-0 w-full space-y-4">
          ...
          <p className="text-xs text-center text-slate-500 dark:text-slate-400">
            Đã có tài khoản?{" "}
            <Link
              to={`/login?redirect=${encodeURIComponent(afterRegister)}`}
